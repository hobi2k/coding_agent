...
사용법:
    uv run agent-server                                     # 기본 모델
    uv run agent-server --model Qwen/Qwen2.5-Coder-14B-Instruct
...
def main():
    ...
    parser.add_argument("--gpu-util", default=0.90, type=float, help="GPU 메모리 사용률 (0~1)")
    parser.add_argument("--max-len",  default=8192, type=int, help="최대 컨텍스트 길이")
    ...
    # Windows에서는 vllm이 WSL 내부에서 실행돼야 하므로 안내
    if IS_WINDOWS:
        print("⚠  Windows 감지: vllm은 WSL(Ubuntu) 또는 Linux에서 실행해야 합니다.")
        print("   WSL 터미널을 열고 동일 명령어를 실행하세요.")
        sys.exit(1)
    ...
    cmd = [
        ...
        "--download-dir",           str(MODELS_DIR / "hub"),  # 모델 캐시 위치 고정
    ]
